...
class SettingForm(ttk.LabelFrame):
    ...
    def __init__(self, master, settings: dict[str, tk.Variable]):
        ...
        # contrast adjustment
        frame_contrast = ttk.Frame(self)
        ttk.Label(frame_contrast, text="明るさ補正：").grid(column=0, row=0, padx=5)
        tk.Entry(frame_contrast, textvariable=settings["brightness"], width=5).grid(column=1, row=0)
        frame_contrast.grid(column=0, row=0, padx=10, pady=5, sticky=tk.W)
        ...
        frame_gamma.grid(column=0, row=1, padx=10, pady=5, sticky=tk.W)

        frame_gamma_target = ttk.Frame(self)
        ttk.Label(frame_gamma_target, text="補正の対象：").grid(column=0, row=0, rowspan=2)
        ...
        frame_gamma_target.grid(column=1, row=1, padx=10, pady=5)
